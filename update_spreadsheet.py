...
def update_spreadsheet():
    try:
        ...
        # Load CSV and overwrite spreadsheet data
        df = pd.read_csv(CSV_PATH)
        
        # Clear existing data in sheet (avoiding old rows lingering)
        for row in sheet.iter_rows(min_row=2, max_row=sheet.max_row, min_col=1, max_col=sheet.max_column):
            for cell in row:
                cell.value = None
        
        # Write new data from CSV to sheet
        for r_idx, row in enumerate(df.itertuples(index=False), start=2):  # Start from row 2 to keep headers
            for c_idx, value in enumerate(row, start=1):
                sheet.cell(row=r_idx, column=c_idx, value=value)

        # Force an update so Git detects changes
